...
def create_frames(castello, taverna, scarto):
    ...
    global frame_taverna
    frame_taverna = tk.Frame(win, width = 450, height = 300, bg = "#106040")
    frame_taverna.pack()
    frame_taverna.place(anchor='center', relx=0.3, rely=0.5)
    show_card(frame_taverna, taverna)

    global frame_castello
    frame_castello = tk.Frame(win, width = 450, height = 300, bg = "#106040")
    frame_castello.pack()
    frame_castello.place(anchor='center', relx=0.5, rely=0.5)
    show_card(frame_castello, castello)

    global frame_scarti
    frame_scarti = tk.Frame(win, width = 450, height = 300, bg = "#106040")
    frame_scarti.pack()
...
def card_clicked(event, card):
    ...
    if card == "K_cuori":
        change_card(frame_castello, "K_picche")

def change_card(frame, new_card):
    try:
        card_coordinates = coordinate_carte[new_card.split("_")[1]][new_card]
        # Define the region you want to display (left, top, right, bottom)
        # Adjust these values as needed to show the desired part of the image
        left, top = card_coordinates[0], card_coordinates[1]
        carta = (left, top, left + 410, top + 570)
        ...
        # Remove the current image from the frame
        for widget in frame.winfo_children():
            widget.destroy()

        # Create a Label Widget to display the new cropped image
        label = tk.Label(frame, image=img, bd=0)
        label.image = img  # Save a reference to the image to prevent garbage collection
        label.pack()

        # Bind the card click event to a function
        label.bind("<Button-1>", lambda event: card_clicked(event, new_card))

    except Exception as e:
        print("Errore durante il cambio della carta:", e)
