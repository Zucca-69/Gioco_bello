...
class Game: 
    ...
    def getGiocatoreTurno(self):
        return self.__giocatore_turno

    def getEnemy(self):
        return self.__nemico

    def gioco(self):
        #ciclo ci gioco
        while self.__re > 0 and self.__continua:
            for pos_giocatore in range(len(self.__giocatori)):
                self.__giocatore_turno = self.__giocatori[pos_giocatore]
                self.__animale = True
                self.__sconfittoNelTurno = False

                # ottieni prima azione 


                # morte -> fine gioco
                if self.turno(self.__giocatore_turno) == False:
                    break

    def turno(self, giocatore):
        # scegli carte da giocare
        lista_giocate = self.gioca_carte(giocatore)

        # attacco al nemico
        self.attacca_nemico(lista_giocate)

        # controlle se un nemico è stato eliminato nel turno 
        self.check_nemico_eliminato_turno()

    def rinuncia(self):
        return True
        
    def gioca_carte(self, giocatore):
        lista_giocate = []
        tot = 0
            ...
            if int(card[0]) >= 2 and int(card[0]) <= 5:
                ...
                while tot + int(card[0]) <= 10 and len(giocabili) > 0 and vuoleGiocare:
                    ...
                    if rinuncia == "" or rinuncia[0].lower() != "s":
                        ...
                        vuoleGiocare = False
        return lista_giocate

    def attacca_nemico(self, lista_giocate):
        # attacco al nemico
    ...
    def check_nemico_eliminato_turno(self):
